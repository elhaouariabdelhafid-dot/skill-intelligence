...
// Seuil de maitrise : valeur de repli, remplacee au demarrage par celle
// configuree par l'administrateur (GET /api/config/public).
let THRESHOLD = 60;

function useThreshold(api, tick = 0) {
...
function MonParcours() {
  ...
  const gaps = plan.data?.recommendations || plan.data?.gaps || plan.data?.plan || [];
              ...
              <li key={g.service || i} className="rise" style={{ animationDelay: `${0.08 + i * 0.06}s` }}>
                ...
                <div className="path-body">
                  <div className="path-head">
                    ...
                    <span className="mono" style={{ color: BAND_COLOR[band(g.current_level ?? g.score ?? 0)] }}>
                      {(g.current_level ?? g.score ?? 0).toFixed(0)}%</span>
                  </div>
                    ...
                    <ul className="res-list">
                      {g.resources.slice(0, 3).map((r, k) => (
                        <li key={k}><BookOpen size={12} strokeWidth={2.2} />
                          <span>{(r.source || "Ressource").split("/").pop()}</span>
...
function EquipeVue() {
  ...
  // Seuls les collaborateurs evalues entrent dans les statistiques
      ...
      {data.map((p, i) => (
        <div key={p.user_id} className="person-row rise" style={{ animationDelay: `${0.06 * i}s` }}>
          <div className="person-id">
            ...
            <div><strong>{p.name}</strong><span className="dim">
...
function Besoins() {
  ...
  // Une personne non evaluee n'est pas une lacune : elle n'a pas encore ete mesuree
  const evalues = data.filter((p) => (p.n_evaluations || 0) > 0);
...
function Resultats() {
  const { data, error, loading, reload } = useApi("/api/evaluations?limit=500");
...
const NAV = {
  ...
  admin:         ["acces", "motsdepasse","comptes", "configuration", "corpus", "services", "audit"],
};
...
export default function App() {
  ...
  const threshold = useThreshold(api, cfgTick);   // aligne l'affichage sur la config serveur
  ...
  return (
    <Ctx.Provider value={{ dark, token: session?.token, api, threshold,
